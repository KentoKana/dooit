...
export class UpdateProjectItemTitleColumnToHeading1634276405005 implements MigrationInterface {
    name = 'UpdateProjectItemTitleColumnToHeading1634276405005'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`doo\`.\`project_items\` CHANGE \`title\` \`heading\` varchar(500) NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`doo\`.\`project_items\` ADD CONSTRAINT \`FK_075bc125384418c6da0a624d808\` FOREIGN KEY (\`projectId\`) REFERENCES \`doo\`.\`projects\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`doo\`.\`project_items\` CHANGE \`heading\` \`title\` varchar(500) NOT NULL`);
    }
