...
export class RemoveNameColumnsAndAddDisplayName1638607760406 implements MigrationInterface {
    name = 'RemoveNameColumnsAndAddDisplayName1638607760406'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`doo\`.\`users\` ADD \`displayName\` varchar(100) NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`doo\`.\`profiles\` ADD CONSTRAINT \`FK_315ecd98bd1a42dcf2ec4e2e985\` FOREIGN KEY (\`userId\`) REFERENCES \`doo\`.\`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`doo\`.\`users\` DROP COLUMN \`displayName\``);
