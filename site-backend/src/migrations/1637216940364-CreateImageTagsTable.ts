...
export class CreateImageTagsTable1637216940364 implements MigrationInterface {
    name = 'CreateImageTagsTable1637216940364'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`doo\`.\`imageTags\` (\`id\` int NOT NULL AUTO_INCREMENT, \`width\` int NOT NULL, \`xCoordinate\` int NOT NULL, \`yCoordinate\` int NOT NULL, \`dateCreated\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`dateModified\` datetime NOT NULL, \`projectItemId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`doo\`.\`imageTags\` ADD CONSTRAINT \`FK_e94f80bb1fcfc4a76e60e1fcf71\` FOREIGN KEY (\`projectItemId\`) REFERENCES \`doo\`.\`project_items\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`doo\`.\`imageTags\` DROP FOREIGN KEY \`FK_e94f80bb1fcfc4a76e60e1fcf71\``);
        await queryRunner.query(`ALTER TABLE \`doo\`.\`project_items\` DROP FOREIGN KEY \`FK_075bc125384418c6da0a624d808\``);
        await queryRunner.query(`ALTER TABLE \`doo\`.\`projects\` DROP FOREIGN KEY \`FK_361a53ae58ef7034adc3c06f09f\``);
        await queryRunner.query(`ALTER TABLE \`doo\`.\`profiles\` DROP FOREIGN KEY \`FK_315ecd98bd1a42dcf2ec4e2e985\``);
        ...
        await queryRunner.query(`ALTER TABLE \`doo\`.\`profiles\` ADD CONSTRAINT \`FK_315ecd98bd1a42dcf2ec4e2e985\` FOREIGN KEY (\`userId\`) REFERENCES \`doo\`.\`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`DROP TABLE \`doo\`.\`imageTags\``);
    }
